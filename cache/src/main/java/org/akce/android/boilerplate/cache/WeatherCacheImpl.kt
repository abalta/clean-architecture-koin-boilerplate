package org.akce.android.boilerplate.cache
 ...
 */
class WeatherCacheImpl constructor(val bufferoosDatabase: BufferoosDatabase,
    ...
    override fun deleteCity(city: Request): Completable {
        return Completable.defer{
            bufferoosDatabase.cachedBufferooDao().deleteCity(
                    entityMapper.mapToCached(city)
            )
            Completable.complete()
        }
    }
     ...
     */
    override fun saveBufferoos(cities: Request): Completable {
        return Completable.defer {
                bufferoosDatabase.cachedBufferooDao().insertBufferoo(
                        entityMapper.mapToCached(cities))
